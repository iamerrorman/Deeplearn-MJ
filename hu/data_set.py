...
class Dataset_hu(Dataset):
    def __init__(self, root, mode, action_dict):
        super(Dataset_hu, self).__init__()
        self.root = root
        ...
        for item in file_names:
            ...
            if item == "hu":
                temp = temp[int(len(temp) * 0.4):]  # pass就取一半来训练，另一半用来测试
...
if __name__ == '__main__':
    root = "D:\\project\\python\\MJDecisionAI-DeepLearn\\data\\output\\hu"
    ds_train = Dataset_gang(root, "train", action_to_id)
