...
class Game:
    def __init__(self):
        ...
        for i in range(0, 4):
            if self.leader_id == i:
                self.player_s[i] = player_quick.Player(self.tiles[0: 14])
                self.tiles = self.tiles[14: len(self.tiles)]
            else:
                self.player_s[i] = player_quick.Player(self.tiles[0: 13])
    ...
    def other_peng_gang_chow(self, last_discard):
        ...
        for item in ids:
            ...
            if self.human_player_id == item:  # 如果当前玩家为人类玩家
                ...
                # quick_hu = (True if item == 0 else False)
                action, operate_card = self.player_s[item].update_status(feature, is_human=False, quick_hu=True)  # 做出决策并更新状态
    ...
    def start(self):
        while True:
            ...
            if self.current_id == self.human_player_id:
                ...
                # quick_hu = (True if self.current_id == 0 else False)
                # print(quick_hu)
                action, operate_card = self.player_s[self.current_id].update_status(self.zip_feature(self.current_id), is_human=False, quick_hu=True)
