...
class Player:
    ...
    def play_by_ai(self, status, quick_hu=False):
        action = []
        ...
        if quick_hu:  # 快速胡牌模式
            # print("**************kuaishu***************")
            # for item in res_action_list:  # 在所有的行为决策中找到不是过的决策行为
            #     if item[0] == "hu":
            #         return "hu"
            res_action_list = sorted(res_action_list, key=lambda x: -x[1])
            if len(res_action_list) == 0:
                return "pass"
            else:
                return res_action_list[0][0]
    ...
    def update_status(self, status, is_human=False, quick_hu=False):
        action = None
        if is_human:
            action = self.play_by_human(status)  # 获取人的决策
        else:
            action = self.play_by_ai(status, quick_hu)  # 获取神经网络输出
        part_act = action.split("_")  # 分割行为字符串
        if len(part_act) == 2 and part_act[0] == "discard":  # 该行为为弃牌
            operate_card = translate33_16(int(part_act[1]))  # 获取操作牌，并转换为16进制手牌
            self.handcards.remove(operate_card)  # 手牌中移除丢弃的牌
